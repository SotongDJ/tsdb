# tsdb

A command-line database runner for DOTSV flat files.

---

## Overview

`tsdb` reads an action file line-by-line and applies each operation to a DOTSV (`.dov`) database file. It is designed for high throughput, minimal memory use, and clean git diffs.

```
tsdb <target.dov> <action.atv>            apply actions to database
tsdb <target.dov> --compact               compact pending section
tsdb --relate <target.dov>                generate .kv.rtv, .vk.rtv, .uuid.rtv indexes
tsdb --plane <target.dov>                 generate kv.ptv, vk.ptv, uuid.ptv, ord.ptv, kt.ptv indexes
tsdb --query <query.qtv> <target.dov>     query database, print matching UUIDs
tsdb --filter <filter.ftv> <target.dov>   rich predicate filter (eq/ne/lt/.. + numeric variants)
tsdb --query  ... --show [<out.dtv>|-]    emit full records (stdout default; '-' alias)
tsdb --filter ... --show [<out.dtv>|-]    emit full records (stdout default; '-' alias)
tsdb --records <uuids.utv|-> <target.dov> print matching records as JSONL on stdout
```

---

## DOTSV File Format

Each record occupies exactly one line:

```
<12-char-UUID>\t<key=value>\t<key=value>...\n
```

Files are plain UTF-8 text — readable with `grep`, `awk`, `diff`, or any editor.

### Two-Section Structure

```
<sorted section>          ← binary-searchable, lexicographic UUID order
                          ← blank line separator
<pending section>         ← write-ahead log, O(1) appends
```

The sorted section supports O(log n) lookup. The pending section is a write-ahead log compacted into the sorted section when it exceeds 100 lines. Compaction is a single sequential O(n) pass.

### UUID Column

The first column is always a **12-character base62-Gu UUID** — a time-sortable, class-prefixed identifier:

```
{C}{G}{century}{YY}{M}{D}{h}{m}{s}{XX}
 1   1    1      2   1  1  1  1  1   2   = 12 chars
```

- `C` — uppercase class prefix (user-defined record type)
- `G` — fixed format marker
- Timestamp encoded in Format-G 60-char alphabet (standard base62 minus `l` and `O`)
- `XX` — 2-char order suffix for collision resolution within the same second

UUIDs sort lexicographically by class, then chronologically within each class. `tsdb` validates but never generates UUIDs — they are always user-supplied.

### Escaping

Only five bytes require escaping inside keys or values:

| Byte | Escaped |
|------|---------|
| `\t` | `\x09` |
| `\n` | `\x0A` |
| `\r` | `\x0D` |
| `=`  | `\x3D` |
| `\`  | `\\`   |

Everything else — CJK, emoji, accented Latin, spaces, commas, quotes — passes through unescaped.

### Arrays

Multi-valued fields use **repeated keys** in the action file and a canonical
JSON-style array on disk:

| Layer        | Form                                           |
|--------------|------------------------------------------------|
| atv input    | `role=admin\trole=editor\trole=viewer`         |
| dov on disk  | `role=["admin","editor","viewer"]`             |
| `--plane`    | one row per element                            |
| `--relate`   | one row per `(key,value)` — array stays packed |

Inside an array element, only `"` and `\` are escaped (`\"`, `\\`); commas,
brackets, and Unicode pass through. Nested arrays and objects are not
supported — the action file rejects any single value shaped like `[...]` or
`{...}`, so collections must be expressed through repeated keys.

---

## Action File

An action file is UTF-8 text. Each line is one operation, using the same format as the DOTSV pending section:

```
# comment (ignored)

+NGk26cHcv001	name=Alice	city=東京	age=30
+NGk26cHdn002	name=Bob	city=大阪

~NGk26cHcv001	city=京都	age=31

-NGk26cHdn002

!EGk26cICK001	name=Carol	city=London
```

### Opcodes

| Opcode | Name   | Behavior                                         |
|--------|--------|--------------------------------------------------|
| `+`    | Append | Insert new record — error if UUID exists         |
| `-`    | Delete | Remove record — error if UUID missing            |
| `~`    | Patch  | Update named fields — error if UUID missing      |
| `!`    | Upsert | Full replace if exists, insert if not — no error |

**Patch semantics:** list only the fields to change. To delete a field, set its value to `\x00`. A patch that would remove all fields is rejected.

**Strict mode (default):** any conflict aborts the run. The `.dov` file is not modified until all operations are validated.

---

## Common Operations

### Append a record

Create `input.txt` with a `+` line — one tab-separated field per column after the UUID:

```
+NGk26cHcv001	name=Alice	city=東京	age=30
```

```bash
tsdb mydb.dov input.txt
```

Errors if `NGk26cHcv001` already exists.

### Modify a record

Use `~` and list only the fields to change. Unlisted fields are left untouched:

```
~NGk26cHcv001	city=京都	age=31
```

To delete a specific field, set its value to `\x00`:

```
~NGk26cHcv001	age=\x00
```

Errors if the UUID does not exist.

### Delete a record

Use `-` with just the UUID:

```
-NGk26cHcv001
```

Errors if the UUID does not exist.

### Combining operations

Multiple operations in one file are validated together, then applied atomically:

```
+PGk26cHcv001	name=Dave	role=admin
~NGk26cHcv001	city=大阪
-NGk26cHdn002
```

```bash
tsdb mydb.dov input.txt
```

### Array fields

Repeat the key for each element. The on-disk record uses the canonical form:

```
# input
+PGk26cHcv001	name=Dave	role=admin	role=editor	role=viewer

# stored on disk as
PGk26cHcv001	name=Dave	role=["admin","editor","viewer"]
```

Patches replace the whole field — supplying `role=admin\trole=ops` overwrites
any previous list. To clear an array field, set it to `\x00` like any other
field. Values shaped like `[...]` or `{...}` are rejected by the parser.

---

## `--relate` — Build Inverted Indexes

`--relate` compacts a `.dov` file and generates three companion index files that support fast key/value lookups:

```bash
tsdb --relate users.dov
# creates: users.kv.rtv    (key → value → UUIDs)
#          users.vk.rtv    (value → key → UUIDs)
#          users.uuid.rtv  (sorted list of all UUIDs)
```

Each index is a plain tab-separated file with exactly three columns:

```
# users.kv.rtv
city	London	EGk26cICK001
city	Tokyo	NGk26cHcv001,NGk26cHdn002
name	Alice	NGk26cHcv001
name	Bob	NGk26cHdn002
# 20260329143022
```

The final line is a timestamp matching the source `.dov`. If all three index files are already current (same timestamp), `--relate` is a no-op — safe to call before every query.

The `.uuid.rtv` companion is a flat sorted list of every UUID in the database, one per line, terminated by the same timestamp footer. It is regenerated alongside the two inverted indexes and shares the same skip-if-current check.

**Array values stay packed** in `.rtv` files: a record with
`role=["admin","editor"]` produces one row with `["admin","editor"]` in
column 2, not one row per element. Use `--plane` for per-element rows.

---

## `--plane` — Build Flat Inverted Indexes

`--plane` is the flattened counterpart of `--relate`. Each UUID occupies its own row, and canonical array values are **expanded into one row per element**:

```bash
tsdb --plane users.dov
# creates: users.kv.ptv    (one row per key/value/UUID triple)
#          users.vk.ptv    (one row per value/key/UUID triple)
#          users.uuid.ptv  (sorted list of all UUIDs)
#          users.ord.ptv   (numeric companion plane index, v0.5)
#          users.kt.ptv    (key-type plane index, v0.6)
```

The skip-if-current rule is **all five** files: if any one is missing or
stale, all five are rewritten. The four legacy bodies remain
byte-identical across the v0.5→v0.6 upgrade boundary; only `*.kt.ptv`
is genuinely new content.

Given a record `+PGk26cHcv001\tname=Dave\trole=admin\trole=editor\trole=viewer`:

```
# users.kv.ptv
city	London	EGk26cICK001
city	Tokyo	NGk26cHcv001
city	Tokyo	NGk26cHdn002
name	Alice	NGk26cHcv001
name	Bob	NGk26cHdn002
name	Dave	PGk26cHcv001
role	admin	PGk26cHcv001
role	editor	PGk26cHcv001
role	viewer	PGk26cHcv001
# 20260329143022
```

The `["admin","editor","viewer"]` stored on disk has been split into three rows; literal commas and quotes inside elements survive the round-trip. A malformed canonical array in the `.dov` aborts the run with a parse error rather than producing a corrupt index.

Choose `--plane` when downstream tools want one record per line (`join`, `sort -u`, `awk`) or when you need to filter on individual array elements; choose `--relate` when compact binary search is the priority. The two modes write to separate files and each has its own skip-if-current check.

---

## `--query` — Filter Records by Key / Value

`--query` reads a `.qtv` query file, auto-runs `--relate`, then prints to stdout the UUIDs of every record that satisfies the criteria.

**Query file format** (`lookup.qtv`):

```
# mode	union
city	Tokyo
name	Alice
```

- First line (optional): `# mode\tunion` or `# mode\tintersect` (default: intersect)
- Criterion lines:
  - `key\tvalue` — records where that exact key=value pair exists
  - bare token — records where the token appears as either a key **or** a value

```bash
tsdb --query lookup.qtv users.dov
# stdout (union mode — Tokyo OR Alice):
NGk26cHcv001
NGk26cHdn002
```

```bash
# intersect mode — must satisfy ALL criteria
cat > exact.qtv << 'EOF'
# mode	intersect
city	Tokyo
name	Alice
EOF
tsdb --query exact.qtv users.dov
# stdout:
NGk26cHcv001
```

### Typical workflow

```bash
# 1. Build / update indexes once after any writes
tsdb --relate users.dov

# 2. Run queries (--relate is also auto-invoked, but skipped when current)
tsdb --query find-admins.qtv users.dov | while read uuid; do
    grep "^$uuid" users.dov
done
```

---

## `--records` — Emit Full Records as JSONL

`--records` (v0.6) takes a UUID list (from a `.utv` file, or `-` for
stdin) and emits one JSON object per line on stdout:

```bash
tsdb --query find-admins.qtv users.dov | tsdb --records - users.dov | jq .
```

Per-line shape: `{"_uuid":"<u>","<k>":<v>,...}\n`. `_uuid` is always
the first member; remaining keys appear in raw-key lex order. Values
are coerced via the same classifier that powers `*.kt.ptv`:

| Class       | JSON encoding                                 |
|-------------|-----------------------------------------------|
| `string`    | JSON string (DOTSV-unescape, then JSON-escape) |
| `number`    | JSON number (verbatim — `30.50` stays `30.50`) |
| `boolean`   | JSON literal `true` / `false`                 |
| `array`     | JSON array of JSON strings                    |
| `timestamp` | JSON string (preserves the 14-digit form)     |

UUIDs absent from the database emit a sentinel line in input position:
`{"_uuid":"ZZZmissing00","_missing":true}`.

The `*.utv` input format is one 12-char base62-Gu UUID per line; blank
lines and `#…` comments are skipped. CRLF line endings are rejected;
convert with `dos2unix` first. The format is byte-compatible with the
output of `tsdb --query` and `tsdb --filter`, so the typical pipeline
above works without `sed` or `awk` wrangling.

---

## Configuration — `.tsdb.toml`

`tsdb` runs with no configuration by default. An optional `.tsdb.toml` file is
searched for in two places, in precedence order:

| Level | Path | Notes |
|-------|------|-------|
| Project | `./.tsdb.toml` | Current working directory |
| User | `~/.tsdb.toml` | Falls back here only if no project file exists |

The first file found wins outright — settings are **not** merged across levels.
When neither exists, built-in defaults apply and behaviour is identical to
releases before v0.7.

### Settings

| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `utc_offset` | string | `"Z"` | Offset applied to the timestamp trailer written at the end of every `.dov` file |

```toml
# .tsdb.toml
utc_offset = "+08:00"
```

Accepted `utc_offset` values: `Z`, `z`, `UTC`, `utc` (all zero), or a signed
offset written as `+HH:MM`, `-HH:MM`, `+HHMM`, `-HHMM`, `+HH` or `-HH`. Hours
must be two digits in `00`–`24` and minutes two digits in `00`–`59`, and the
total may not exceed 24 hours either way. The sign is required, and a dangling
colon (`+08:`) is rejected rather than read as `+08:00`.

The bound is deliberately wider than the real-world range of `-12:00` to
`+14:00`: this setting shifts a comment line, it is not a timezone database and
never names a zone, so hard-coding political boundaries would reject legitimate
values like `+13:45` for no benefit.

Without this setting the trailer is written in UTC, which can read hours away
from the wall clock of whoever is looking at the file. Setting `utc_offset` to
your own offset makes `# YYYYMMDDhhmmss` match local time. Only the trailer is
affected — record contents are never rewritten, and timestamp *values* stored
inside records are left exactly as supplied.

**The trailer carries no offset marker**, so it is a local convenience, not a
portable instant. Two files written either side of a configuration change are
indistinguishable, and a `.dov` file moved to another machine reads as a wall
clock that machine cannot identify. If you need portable instants, keep
`utc_offset` at its `Z` default.

### Grammar

A deliberately small, strict subset of TOML: a flat table of scalar settings.

```toml
# full-line comment
key = "value"        # double-quoted string
key = value          # bare value
```

Blank lines and full-line comments are ignored. A `#` after a value is **not**
treated as a trailing comment — it is part of the value, so `utc_offset =
"+08:00" # local` is an error, not a setting.

Unknown keys, malformed lines and unparsable values are all rejected, and the
file is never partially applied. The error is reported on stderr — naming the
file, the line number and the offending value — and the built-in defaults are
used, because timestamp formatting sits on the write path and must never fail a
database write. A `.tsdb.toml` that is not a regular file (a directory of that
name) is skipped as though absent.

Note for unattended use: since a bad configuration degrades to a warning rather
than an error, a typo in a `tsdb` run whose stderr is discarded (cron, for
instance) leaves the trailer silently in UTC. Check the setting interactively
once after editing it.

---

## Concurrency

Multiple `tsdb` instances targeting the same `.dov` file are coordinated through a companion lock file (`target.dov.lock`).

### Protocol

1. Pre-scan action file → collect all target UUIDs
2. `flock` the `.lock` file exclusively (held for microseconds)
3. Check UUID set intersection against all queued entries — **reject immediately on any overlap**
4. Append a `WAIT` entry to the queue manifest; release lock
5. Poll until promoted to `EXEC` (no blocking EXEC with overlapping UUIDs ahead)
6. Execute; refresh timestamp periodically
7. Atomic write: `target.dov.tmp` → rename → `target.dov`
8. Remove own entry from queue manifest

### Lock File Format

```
EXEC\t<16-hex-pid>\t<uuid1,uuid2,...>\t<unix-timestamp>\n
WAIT\t<16-hex-pid>\t<uuid1,uuid2,...>\t<unix-timestamp>\n
```

Instances with **non-overlapping UUID sets can run concurrently**. Stale entries (timestamp > 30 seconds old) are evicted automatically. Compact-only mode (`--compact`) treats its UUID set as "all records" and waits for all other writers to finish.

---

## Companion Files

| File | Purpose |
|------|---------|
| `target.dov` | Database |
| `target.dov.lock` | Queue manifest and `flock` target |
| `target.dov.tmp` | Transient during atomic write (auto-cleaned) |
| `target.kv.rtv` | Key-value inverted index (generated by `--relate`) |
| `target.vk.rtv` | Value-key inverted index (generated by `--relate`) |
| `target.uuid.rtv` | Sorted UUID list (generated by `--relate`) |
| `target.kv.ptv` | Key-value flat index (generated by `--plane`) |
| `target.vk.ptv` | Value-key flat index (generated by `--plane`) |
| `target.uuid.ptv` | Sorted UUID list (generated by `--plane`) |
| `target.ord.ptv` | Numeric companion plane index (generated by `--plane`, v0.5) |
| `target.kt.ptv` | Key-type plane index (generated by `--plane`, v0.6) |
| `target.dtv` | Display TSV — full records (generated by `--show <file>`) |
| `<input>.utv` | UUID-list input for `--records` (user-authored or piped) |
| `.tsdb.toml` | Optional configuration, project or user level (v0.7) |

---

## Building

Requires Rust 1.94+.

```bash
source "$HOME/.cargo/env"
cargo build --release
# binary: target/release/tsdb
```

Dependencies: `fs2` (cross-platform flock), `rand` (random process ID).

---

## Whitepapers and References

Full format specifications are in `_ref/`:

- `_ref/DOTSV-whitepaper.md` — file format, escaping, binary search, concurrency protocol, timestamp tracking
- `_ref/tsdb-whitepaper.md` — CLI opcodes, execution model, `--relate`, `--plane`, `--query`
- `_ref/base62-whitepaper.md` — base62-Gu UUID encoding system
- `_ref/extensions-technote.md` — technical note for atsv, rtsv, ptsv, qtsv formats
