...
2. **Read timestamp** — read the `# YYYYMMDDhhmmss` comment from the last line of `<target.dov>`.
...
7. **Append timestamp footer** — write `# YYYYMMDDhhmmss` as the final line of each `.rtv` file, using the value read from the `.dov` in step 2.
...
7. **Append timestamp footer** — write `# YYYYMMDDhhmmss` as the final line of each `.ptv` file, using the value read from the `.dov` in step 2.
...
Rows are sorted lexicographically on col 1, then col 2, enabling O(log n) binary search. Canonical array values from the source `.dov` are kept packed in column 2; use `ptsv` if per-element rows are needed. The last line is a `# YYYYMMDDhhmmss` timestamp matching the source `.dov`. Not hand-authored.
...
The last line is a `# YYYYMMDDhhmmss` footer matching the source `.dov`.
...
Footer: `# YYYYMMDDhhmmss`, exact-byte copy of the `.dov` footer at execution time.
...
`# YYYYMMDDhhmmss` footer matching the source `.dov`.
...
EGk26cICK001	name=Carol	created=20260329143022	roles=["editor","viewer"]
```
