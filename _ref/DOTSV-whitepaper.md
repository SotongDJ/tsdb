...
Rows are sorted lexicographically by column 1 then column 2, enabling O(log n) binary search. The UUID array in column 3 is `,`-separated (no spaces). Canonical array values in the source `.dov` (§4.1) are **kept packed** in column 2 — a row for `role=["admin","editor"]` has `["admin","editor"]` as col 2, not two separate rows. Use `ptsv` (§`--plane`) if per-element rows are needed. The last line is a `# YYYYDDMMhhmmss` timestamp matching the source `.dov` (see §15). Generated by `tsdb --relate`; never hand-authored.
...
Rows are sorted lexicographically on `(col 1, col 2, col 3)`. The last line is a `# YYYYDDMMhhmmss` timestamp matching the source `.dov` (see §15). Generated by `tsdb --plane`; never hand-authored. Optimised for shell pipelines (`awk`, `sort -u`, `join`) that assume one record per line.
