...
A fully flattened (denormalised) sibling of `rtsv`. Instead of a comma-separated UUID array in column 3, every `(col1, col2, uuid)` triple occupies its own row, so a single `.rtv` row with a *j*-element UUID array expands to *j* `.ptv` rows. Two variants are generated per `.dov` file:
...
Rows are sorted lexicographically on `(col 1, col 2, col 3)` — the same order `rtsv` would produce if its comma lists were expanded and split. The last line is a `# YYYYDDMMhhmmss` timestamp matching the source `.dov` (see §15). Generated by `tsdb --plane`; never hand-authored. Optimised for shell pipelines (`awk`, `sort -u`, `join`) that assume one record per line.
