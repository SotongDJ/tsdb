...
- 0.3 — ptsv plane-index format
- 0.4 — canonical array values; `--plane` expands arrays into per-element rows
- 0.5 — timestamp field order corrected to `YYYYMMDD` (month before day, restoring the §15 string-sortability guarantee; implemented in tsdb 0.6.1)
...
- A key MUST NOT appear more than once in a single record. Multi-valued fields are represented as a single canonical array value (see §4.1).

### 4.1 Array Values

Multi-valued fields are stored as a single value in **canonical array form**:

```
role=["admin","editor","viewer"]
```

The canonical form is a JSON-style array of double-quoted string elements:
...
Inside an element, only two bytes are escaped — `"` as `\"` and `\` as `\\`.
Everything else (including commas, square brackets, CJK, emoji) passes through
unchanged. The element layer is independent of the outer DOTSV escape layer:
the canonical array string is written into the record like any other value,
and the record's DOTSV-layer escaping (§5) is then applied to the whole thing.

An array value is detected by shape: the trimmed value starts with `[` and
ends with `]`. Values that happen to match this shape by coincidence are not
allowed to enter the database — the action file parser rejects any single
value shaped like `[...]` or `{...}` so that arrays are produced only via
repeated keys in the action file, never smuggled through a scalar.

Nested arrays and objects are **not** supported. Elements are plain strings.

Array values are written by the `tsdb` action-file processor and consumed
by `tsdb --plane`, which expands each element into its own `(key, value, uuid)`
row. `tsdb --relate` preserves array values intact in column 2 of the `.rtv`
index (it does not split them).
...
The DOTSV ecosystem defines four companion file formats that share the same UTF-8 plain-text conventions:
...
| `rtsv` | `*.rtv`   | Relation Tab Separated Vehicle   | Generated inverted index (UUID array in col 3)     | No             |
| `ptsv` | `*.ptv`   | Plane Tab Separated Vehicle      | Flattened inverted index (one row per UUID)        | No             |
...
Rows are sorted lexicographically by column 1 then column 2, enabling O(log n) binary search. The UUID array in column 3 is `,`-separated (no spaces). Canonical array values in the source `.dov` (§4.1) are **kept packed** in column 2 — a row for `role=["admin","editor"]` has `["admin","editor"]` as col 2, not two separate rows. Use `ptsv` (§`--plane`) if per-element rows are needed. The last line is a `# YYYYMMDDhhmmss` timestamp matching the source `.dov` (see §15). Generated by `tsdb --relate`; never hand-authored.

### `ptsv` (Plane TSV)

A fully flattened (denormalised) sibling of `rtsv`. Two dimensions are expanded:

1. Instead of a comma-separated UUID array in column 3, every `(col1, col2, uuid)` triple occupies its own row. A `.rtv` row with a *j*-element UUID array expands to *j* `.ptv` rows.
2. Canonical array values (§4.1) in column 2 are also split. A source record with `role=["admin","editor","viewer"]` contributes three `(role, <elem>, uuid)` rows rather than a single `(role, ["admin",...], uuid)` row.

Two variants are generated per `.dov` file:

| Variant       | Filename              | Column order     |
|---------------|-----------------------|------------------|
| Key-Value     | `<target>.kv.ptv`    | key, value, uuid |
| Value-Key     | `<target>.vk.ptv`    | value, key, uuid |

Rows are sorted lexicographically on `(col 1, col 2, col 3)`. The last line is a `# YYYYMMDDhhmmss` timestamp matching the source `.dov` (see §15). Generated by `tsdb --plane`; never hand-authored. Optimised for shell pipelines (`awk`, `sort -u`, `join`) that assume one record per line.
